...
def write_config_file_cb(b):
    path_to_share = os.path.join('~', '.local','share','physicellProjectNanohub')
...
def get_config_files():
    ...
    if nanoHUB_flag:
        full_path = os.path.expanduser("~/data/results/.submit_cache/physicellProjectNanohub")  # does Windows like this?
    else:
        # local cache
        try:
            cachedir = os.environ['CACHEDIR']
            full_path = os.path.join(cachedir, "physicellProjectNanohub")
        except:
...
def run_done_func(s, rdir):
    ...
    if nanoHUB_flag:
        ...
        os.system("submit  mail2self -s 'nanoHUB physicellProjectNanohub' -t 'Your Run completed.'&")
...
def run_sim_func(s):
    ...
    if nanoHUB_flag:
        if remote_cb.value:
            s.run(run_name, "-v ncn-hub_M@brown -n 8 -w 1440 physicellProjectNanohub-r7 config.xml")   # "-r7" suffix??
        else:
...
if nanoHUB_flag:
    run_button = Submit(label='Run',
                       start_func=run_sim_func,
                        ...
                        cachename='physicellProjectNanohub',
                        showcache=False,
                        outcb=outcb)
else:
    if (hublib_flag):
...
tool_title = widgets.Label(r'\(\textbf{physicellProjectNanohub}\)')
